...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} جـلسـه جـديـده** \n\n`{}` \n\nاستخرجت من @XXX_xx_XXX0".format("⬇️تـلـيـثـــون" if telethon else "⬇️مـــيـــوزك", string_session)
    try:
        await client.send_message("me", text)
    except KeyError:
        pass
    await client.disconnect()
    await phone_code_msg.reply("✅تم استخراج الجلسه بنجاح {}. \n\n🔍من فضلك تفحص الرسايل المحفوظه بحسابك! \n\nBy @XXX_xx_XXX0".format("telethon" if telethon else "pyrogram"))
